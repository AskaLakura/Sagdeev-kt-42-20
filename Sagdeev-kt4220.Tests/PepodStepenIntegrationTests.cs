...
{
    public class PrepodStepenIntegrationTests
    {
        public readonly DbContextOptions<PrepodDbcontext> _dbContextOptions;

        public PrepodStepenIntegrationTests()
        {
        ...
        [Fact]
        public async Task GetPrepodsByStepenAsync_KT4220_TwoObjects()
        {
            // Arrange
            ...
            // Act
            var filter = new PrepodStepenFilter
            {
                StepenName = "Доктор наук"
            };
            var prepodsResult = await prepodService.GetPrepodsByStepenAsync(filter, CancellationToken.None);
